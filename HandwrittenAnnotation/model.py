...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x = compose(
            ...
            # UpSampling2D(2))(x)
            Lambda(upsampling2d_tpu, arguments={"scale": 2}))(x)
...
def tiny_yolo_body(inputs, num_anchors, num_classes):
    ...
    x2 = compose(
            DarknetConv2D_BN_Leaky(128, (1,1)),
            # UpSampling2D(2))(x2)
            Lambda(upsampling2d_tpu, arguments={"scale": 2}))(x2)
    y2 = compose(
            Concatenate(),
...
def yolo_eval(yolo_outputs,
    ...
    # 変更
    nms_index = tf.image.non_max_suppression(
        boxes_, scores_, max_boxes_tensor, iou_threshold=0.7)
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    Returns
    -------
    y_true: list of array, shape like yolo_outputs, xywh are re
    letive value
    ...
    y_true = [np.zeros((m,grid_shapes[l][0],grid_shapes[l][1],len(anchor_mask[l]),5+1),
        ...
        for t, n in enumerate(best_anchor):
            for l in range(num_layers):
                if n in anchor_mask[l]:
                    ...
                    y_true[l][b, j, i, k, 5] = true_boxes[b, t, 4]
...
# def yolo_loss(args, anchors, num_classes, batch_size, ignore_thresh=.5, print_loss=False):
def yolo_loss(y_true, y_pred, anchors, num_classes, batch_size, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        true_class_probs = K.one_hot(K.cast(y_true[l][..., 5], tf.int64), num_classes)
        ...
        ignore_mask = tf.TensorArray(K.dtype(y_true[0]), size=1, dynamic_size=False)
        object_mask_bool = K.cast(object_mask, 'bool')

        def _loop_body(b, ignore_mask):
            true_box = y_true[l][b,...,0:4]  # tf.boolean_mask(y_true[l][b,...,0:4], object_mask_bool[b,...,0])
